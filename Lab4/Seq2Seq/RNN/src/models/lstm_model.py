...
class LSTMSeq2Seq(BaseSeq2Seq):
    ...
    def forward(self, src, tgt=None):
        ...
        if self.config.bidirectional:
            ...
            cell = cell.view(self.config.num_layers, 2, batch_size, -1)
            cell = cell.transpose(1, 2).contiguous()
            cell = cell.reshape(self.config.num_layers, batch_size, -1)

        if tgt is None:
            return self.generate(src, encoder_outputs, (hidden, cell))

        # 训练模式
        decoder_input = tgt[:, :-1]
        decoder_embedded = self.decoder_embedding(decoder_input)
        decoder_outputs, _ = self.decoder_lstm(decoder_embedded, (hidden, cell))
    ...
    def generate(self, src, encoder_outputs=None, hidden=None, max_len=50):
        ...
        for _ in range(max_len):
            ...
            scores = self.attention(attn_input)  # [batch_size, seq_len, 1]
            attention_weights = torch.softmax(scores, dim=1).transpose(1, 2)  # [batch_size, 1, seq_len]
            context = torch.bmm(attention_weights, encoder_outputs)  # [batch_size, 1, hidden]
